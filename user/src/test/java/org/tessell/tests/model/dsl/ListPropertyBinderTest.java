...
public class ListPropertyBinderTest {
  ...
  public void initialNamesAreAddedToPanel() {
    names.add("one");
    names.add("two");
    bind(names).to(panel, viewFactory);
  ...
  public void initialPresentersAreAddedToPanel() {
    names.add("one");
    names.add("two");
    ParentPresenter parent = doBind(new ParentPresenter());
    bind(names).to(parent, panel, presenterFactory);
    assertLabel(panel.getIsWidget(0), "one");
    assertLabel(panel.getIsWidget(1), "two");
  ...
  private static <P extends Presenter> P doBind(P p) {
    p.bind();
    return p;
  }
}
